...
// Helper function to extract dataset from model reference name
const getDatasetFromModelName = (modelName: string): string | null => {
  // Ensure modelName is a string and not undefined/null
  if (!modelName || typeof modelName !== 'string') {
    return null;
  }

  // For BigQuery multi-dataset, model names are in format: dataset_tablename
  // Extract dataset part before first underscore
...
// Helper function to format model display with dataset context
const formatModelNameWithDataset = (
  modelName: string,
  fieldName: string,
): string => {
  // Add safety checks
  ...
  const dataset = getDatasetFromModelName(modelName);
  if (dataset) {
    return `${modelName}.${fieldName} (${dataset})`;
  }
  return `${modelName}.${fieldName}`;
};
  ...
  const columns: ColumnsType<RelationsDataType> = [
    {
      ...
      key: 'fromField',
      render: (fromField) =>
        formatModelNameWithDataset(fromField.modelName, fromField.fieldName),
      width: '35%',
    },
    {
      title: 'To',
      ...
      key: 'toField',
      render: (toField) =>
        formatModelNameWithDataset(toField.modelName, toField.fieldName),
      width: '35%',
    },
    {
      title: 'Type',
  ...
  return (
    <div className="mt-6">
      <ModelRelationSelectionTable
        ...
        tableTitle={(() => {
          const displayName = recommendNameMapping[modelName] || modelName;
          const dataset = getDatasetFromModelName(modelName);
          return dataset ? `${displayName} (${dataset})` : displayName;
        })()}
